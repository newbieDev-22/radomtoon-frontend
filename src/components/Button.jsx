...
export default function Button({
  ...
  const commonClasses = `flex justify-center items-center transition hover:brightness-110 active:brightness-75 duration-300 font-bold px-4  rounded-md 
                         h-10 md:${heightBtnMap[height]} md:${widthBtnMap[width]} ${bgBtnMap[bg]} ${borderBtnMap[border]} ${colorBtnMap[color]} ${border && "border-4"}`;

  const notActiveClasses = `flex justify-center items-center font-bold px-4  rounded-md h-10 md:${heightBtnMap[height]
